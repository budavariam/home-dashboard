...
const HistoricalChart: React.FC = () => {
    ...
    const createDatasets = (key: "hum" | "tmp" | "bat") =>
        Object.entries(groupedData).map(([device, data]) => ({
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded shadow-lg">
            <div className="flex flex-col md:flex-row gap-4 mb-4">
                ...
                <div className="border-b my-2 border-gray-300 dark:border-gray-600"></div>
                    ...
                    .map((key) => (
                        <div key={key} className="mb-6">
                            <Line
                                options={{
                                    ...chartOptions,
                                    plugins: { ...chartOptions.plugins, title: { ...chartOptions?.plugins?.title ?? "", text: `${key === "hum" ? "Humidity" : key === "tmp" ? "Temperature" : "Battery"} Chart` } },
                                }}
                                data={{
                                    labels: Object.values(groupedData)[0]?.timestamps || [],
